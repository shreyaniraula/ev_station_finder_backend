...
const registerUser = asyncHandler(async (req, res, next) => {
  ...
  if (phoneNumber.length != 10) {
    return res
      .status(401)
      .json(new ApiResponse(401, {}, "Invalid phone number"));
  }
...
const loginUser = asyncHandler(async (req, res, next) => {
  ...
  if (!isPasswordValid) {
    return res
      .status(401)
      .json(new ApiResponse(401, {}, "Incorrect user credentials"));
  }
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ ...user._doc, accessToken: req.token });
});
...
const verifyToken = asyncHandler(async (req, res) => {
  ...
  const verified = Jwt.verify(token, USER_ACCESS_TOKEN_SECRET);
